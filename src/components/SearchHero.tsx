...
interface Props {
  onSearch: (fromPort: string, toPort: string, passengerCount: number, departureDate: string, fareCategory?: FareCategory) => void;
...
export const SearchHero: React.FC<Props> = ({ onSearch, initialFromPort, initialToPort }) => {
  ...
  const handleSearch = (e: React.FormEvent) => {
    ...
    setError(null);
    onSearch(fromPort, toPort, passengerCount, departureDate, fareCategory);
  };
    ...
    <div className="glass-panel rounded-3xl overflow-hidden shadow-xl border border-slate-200/80 text-left">
      ...
      <form onSubmit={handleSearch} className="p-5 md:p-8 space-y-6">
        ...
        <div className="flex justify-end items-center pt-2 flex-wrap gap-4">
          ...
          >
            <Search size={18} /> Search Speedboat Trips
          </button>
        </div>
      </form>
    </div>
